...
describe('MongoMbidCacheRepository', () => {
  ...
  it('upsertTracklist writes document with TTL and returns void', async () => {
    ...
    expect(update.mbid).toBe('m1');
    expect(update.tracklist).toEqual(tracklist);
    expect(update.fetchedAt).toBeInstanceOf(Date);
    expect(update.expiresAt).toBeInstanceOf(Date);
    expect(options).toMatchObject({ upsert: true });

    const diff = update.expiresAt.getTime() - update.fetchedAt.getTime();
